...
func _redraw() -> void:
	...
	var position = region_position * region_size
	...
	if show_rect:
		...
		if abs(region_position.x) > Terrain3DData.REGION_MAP_SIZE*.5 or abs(region_position.y) > Terrain3DData.REGION_MAP_SIZE*.5:
			modulate = Color.GRAY
		draw_rect(Vector2(region_size,region_size)*.5 + position, region_size, selection_material, modulate)
	
	for pos in grid:
		var grid_tile_position = Vector2(pos) * region_size
		if show_rect and grid_tile_position == position:
