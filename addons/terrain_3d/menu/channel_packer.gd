		...
		var lumin_fn: Callable = func() -> void:
			if !images[IMAGE_ALBEDO]:
				...
				if height_texture.is_empty():
					_show_message(ERROR, "Height Texture2D Generation error")
				...
				_show_message(INFO, "Height Texture2D generated sucsessfully")
...
func _on_pack_button_pressed() -> void:
	...
	if packing_albedo:
		...
		save_file_dialog.title = "Save Packed Albedo/Height Texture2D"
	...
	if packing_normal_roughness:
		...
		save_file_dialog.title = "Save Packed Normal/Roughness Texture2D"
		save_file_dialog.popup_centered_ratio()
