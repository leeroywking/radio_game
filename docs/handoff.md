...
- The audio path now treats silence as a first-class state: off-channel and full-quieting conditions are silent, with hiss gated in only during weak-copy moments
...
14. The old always-on static bed was too rough for training and too brittle for browser playback.
   The active audio path now gates hiss instead of running it continuously, and the web export path explicitly resumes the browser audio context on first interaction.
...
- DF audio regression coverage
  The headless testing agent now asserts that the DF path is actually audible on the target broadcast and that full-quieting and off-channel states are silent on the noise path.
